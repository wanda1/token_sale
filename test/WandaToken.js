...
contract('WandaToken', function(accounts){
    ...
    it('initializes the contract with the correct values', function() {
        return WandaToken.deployed().then(function(instance) {
            ...
            assert.equal(name, 'Wanda Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'WANDA', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard) {
            assert.equal(standard, 'Wanda Token v1.0', 'has the correct standard')
        });
    });
